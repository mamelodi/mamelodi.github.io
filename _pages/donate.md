---
title: "Donate"
permalink: /donate/
layout: single
author_profile: true
---

The Mamelodi Initiative requires all types of support to make its programming
possible.  Any donation will contribute to making a difference in a child's
life.  If you would like to make a donation or in-kind gift in a way not listed
below, please email Uyanda Ngobeni (Finance Director) at
[development@mamelodi.org](mailto:development@mamelodi.org).

## If You Are in the US

Click the button below to make an online donation through Network for Good.

[<i class="fas fa-heart"></i> Donate Online (US)](https://donatenow.networkforgood.org/mamelodi){: .btn .btn--primary .btn--large target="_blank" rel="noopener noreferrer"}

Alternatively, you can mail a check to:
```
Mamelodi Initiative Foundation
c/o Michael Silvestri
350 Tappan St, #3
Brookline, MA 02445
```
*Donations will be processed by the US-based 501(c)3.*

---

## If You Are in South Africa

You can donate online through our BackaBuddy campaign for the 2026 Winter Jam (programme runs from 29 June – 18 July):

[<i class="fas fa-hand-holding-heart"></i> Donate via BackaBuddy (RSA)](https://www.backabuddy.co.za/campaign/sponsor-a-learner-for-winter-jam-2026){: .btn .btn--primary .btn--large target="_blank" rel="noopener noreferrer"}


The Mamelodi Initiative is a **Section 18A**-approved organisation; your donation qualifies for a South African tax deduction.

- **NPO:** 126-143
- **PBO:** 930078512

Alternatively, you can donate directly via bank transfer:

### Banking Details

| | |
|---|---|
| **Bank** | First National Bank (FNB) |
| **Branch Code** | 255355 |
| **Account Name** | The Mamelodi Initiative NPO |
| **Account Number** | 62870007575 |
| **Account Type** | Cheque |
| **Swift Code** | FIRNZAJJ (for international transfers) |

*Donations will be processed by the RSA-based RNPO.*
